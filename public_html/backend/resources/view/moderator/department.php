<div class="main-content">
	<div class="main-content-inner">
        ...
        <li class="active"><?=$this->lang->menu->moderator->department?></li>
      </ul>
		</div>
		...
		<div class="page-content">
			<div class="col-xs-12">
				<div class="page-header">
					<h1><?=$this->lang->menu->moderator->department?>
					  <div class="btn-group pull-right">
						  ...
						  <ul class="dropdown-menu">
							  ...
							  <li><a href="<?=$this->baseurl->backend->module->moderator->department?>" id="reset_filter_table"><?=$this->lang->action->reset_filter?></a></li>
						  </ul>
					  </div>// 	<!-- /.btn-group left-->
					</h1>
        </div>// 	<!-- /.page-header -->
       <!--AJAX Message-->
        ...
        <!--AJAX Message-->
          ...
          <tbody>
						...
						?>
              <tr>
								<td class="center"><label class="pos-rel"><input id="id_<?=$value['id']?>" name="items" value="<?=$value['id']?>" type="checkbox" class="ace check_items_all" /><span class="lbl"></span></label></td>
								<td class="center"><?=$value['id']?></td>
                <td><a href="<?=$this->baseurl->front->module->news->department.$value['id']?>" target="_blank"><?=$this->xFunction->htmlspec($value['name'])?></a></td>
							  <td class="center">
									<a href="#modal-edit" onclick="load_form_department(<?=$value['id']?>);" data-toggle="modal" data-backdrop="static" data-keyboard="false"  class="btn btn-primary btn-xs btn-action" title="<?=$this->lang->title->edit?>">
    ...
    <div class="modal-content">
      <form id="frm" name="frm" class="form-horizontal" data-parsley-validate="" role="form" method="post">
        <div id="modal-wizard-container">
          <div class="modal-header"><h3><?=$this->lang->page->department_add?></h3></div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <form id="efrm" name="efrm" class="form-horizontal" data-parsley-validate="" role="form" method="post">
        <div id="modal-wizard-container">
          <div class="modal-header"><h3><?=$this->lang->page->department_edit?></h3></div>
          <div class="modal-body step-content">
						<span id="load_html_department"></span>
          </div><!--modal-body step-content-->
        </div><!--modal-wizard-container-->
        <div class="modal-footer wizard-actions">
          <!--AJAX Message-->
...
<script type="text/javascript">
			$(document).ready(function(){
					$('#check-all').attr('readonly',true);
					$('#check_items_all').attr('readonly',true);
			});
     function load_form_department(id){
       $("#load_html_department").html('<i class="ace-icon fa fa-spinner fa-spin red bigger-130"></i> <?=$this->lang->alert->loading->please_wait?>...');
       $.ajax({
        url : "<?=$this->baseurl->backend->loading?>",
        type: "POST",
        data : {module:'Moderator',load:'department',id:id},
        success: function(data, textStatus, jqXHR){
          if(textStatus=='success'){$( "#load_html_department" ).html(data);}
    ...
    $(function () {
	    ...
	    .on('form:submit', function() {
			    ...
			    formData.append('action', 'department-create');
	    ...
	    .on('form:submit', function() {
			    var formData = new FormData();
			    formData.append('module', 'Moderator');
			    formData.append('action', 'department-update');
						...
						callback: function(result)
						{
							if(result) {
								 $.ajax({
									url : "<?=$this->baseurl->backend->process?>",
									type: "POST",
									data : {module:'Moderator',action:'department-select-del',items_list:numbers},
									success: function(data, textStatus, jqXHR)
									{
